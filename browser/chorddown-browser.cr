...
alias PathChordfileTuple = NamedTuple(
  path: String,
  file: ChordDown::ChordFile | ChordFileError)
channel = Channel(PathChordfileTuple).new(paths.size)
...
paths.each do |path|
  spawn do
    channel.send({path: path, file: readfile(channel, path)})
  end
end

files = Array(PathChordfileTuple).new
(1..paths.size).each do |_|
  files << channel.receive
end

chord_file_count = files.count { |f| f[:file].is_a? ChordDown::ChordFile }
if chord_file_count == 0
  puts "No song files in directory: #{root_dir}"
end
...
class SongList < ScrollList
  property chord_files = [] of NamedTuple(path: String, file: ChordDown::ChordFile)

  def setup_items
    @items.clear
    chord_files.each do |t|
      f = t[:file]
      append_item [
        "{red-fg}",
...
help_browser = "up/down: {bold}j/k{/bold}    top/bottom: {bold}g/G{/bold}    jump {bold}u{/bold}p/{bold}d{/bold}own    {bold}r{/bold}andom    sort by {bold}a{/bold}rtist/{bold}t{/bold}itle    {bold}e{/bold}dit    {bold}q{/bold}uit"
...
class ChorddownBrowser
  ...
  files.each do |v|
    if v[:file].is_a? ChordDown::ChordFile
      songlist.chord_files.push({
        path: v[:path],
        file: v[:file].as(ChordDown::ChordFile),
      })
    end
  end
  songlist.chord_files.sort! do |a, b|
    a[:file].title <=> b[:file].title
  end
  ...
  songlist.on(Event::KeyPress) do |e|
    if e.char == 'j' || e.key == Tput::Key::Down
      ...
      songlist.chord_files.sort! do |a, b|
        a[:file].artists.join.downcase <=> b[:file].artists.join.downcase
      end
      songlist.setup_items
      songlist.select_top
      s.render
      ...
      songlist.chord_files.sort! do |a, b|
        a[:file].title.downcase <=> b[:file].title.downcase
      end
      songlist.setup_items
      songlist.select_top
      s.render
    elsif e.char == 'e'
      Process.run("/usr/bin/env", ["kitty", "kak", songlist.current_song[:path]])
    elsif e.char == 'z'
      s.resize
    end
  end
  ...
  songlist.on(Event::SelectItem) do |e|
    ...
    viewer.song = songlist.current_song[:file]
    s.render
    s.focus viewer
  end

  songlist.on_select do |i|
    f = songlist.chord_files[i][:file]
    c = [] of String
